...
void CSVMatrix::ReorderColumnAfter(
    const std::string& reference_column_name,
    const std::string& column_to_move_name)
{
    ...
    else
    {
        std::string e
        {
            std::string("Error: Cannot reorder column ")
                + std::to_string(column_to_move_index)
                + std::string(" relative to itself. Provided column name is ")
...
void CSVMatrix::ReorderColumnBefore(
    const std::string& reference_column_name,
    const std::string& column_to_move_name)
{
    matrix_row_t::size_type reference_column_index{GetColumnIndex(reference_column_name)};
